void shiftRegisterScan()
{
    ...
    if (SRCOUNT >= 1)
    {
        ...
        for (uint8_t i = 0; i < 8 * SR1CHAIN; i++)
        {
            ...
            uint8_t colNumber = i % 8; //Identify which column we're on
            rawState[rowNumber][i] = digitalRead(SR1DATA); //Set the bit to rawState
    ...
    if (SRCOUNT >= 2)
    {
        ...
        for (uint8_t i = 0; i < 8 * SR2CHAIN; i++)
        {
            ...
            uint8_t colNumber = i % 8; //Identify which column we're on
            rawState[rowNumber][i] = digitalRead(SR2DATA); //Set the bit to rawState
    ...
    if (SRCOUNT >= 3)
    {
        ...
        for (uint8_t i = 0; i < 8 * SR3CHAIN; i++)
        {
            ...
            uint8_t colNumber = i % 8; //Identify which column we're on
            rawState[rowNumber][i] = digitalRead(SR3DATA); //Set the bit to rawState
    ...
    if (SRCOUNT >= 4)
    {
        ...
        for (uint8_t i = 0; i < 8 * SR4CHAIN; i++)
        {
            ...
            uint8_t colNumber = i % 8; //Identify which column we're on
            rawState[rowNumber][i] = digitalRead(SR4DATA); //Set the bit to rawState
